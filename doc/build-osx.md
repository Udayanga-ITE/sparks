...
This guide describes how to build sparksd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Sparks Core from source.
...
### 4. Clone Sparks repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Sparks Core repository to a directory.
...
It is not necessary to build wallet functionality to run `sparksd` or  `sparks-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/sparkspay/sparks/blob/master/doc/descriptors.md), `sqlite` is required.
...
Sparks Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/sparkspay/sparks/issues/7714).
...
You can deploy a `.zip` containing the Sparks Core application using `make deploy`.
...
Run the following in your terminal to compile Sparks Core:
...
Sparks Core should now be available at `./src/sparksd`.
If you compiled support for the GUI, it should be available at `./src/qt/sparks-qt`.

The first time you run `sparksd` or `sparks-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Sparks/
```
...
=======
```shell
./src/sparksd -daemon      # Starts the sparksd daemon.
./src/sparks-cli --help    # Outputs a list of command-line options.
./src/sparks-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/sparks-qt -server # Starts the sparks-qt server mode, allows sparks-cli control
```
