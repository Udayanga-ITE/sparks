When Sparks Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Sparks node is listening,
or one where a service is listening that is not related to the Sparks network.
As a result, this service may occasionally get connection attempts from Sparks
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Sparks Core,
trying to connect because it thinks there is a Sparks node on that
...
They are also considered "bad" ports as they require clients to either run Sparks
...
Below is a list of "bad" ports which Sparks Core avoids when choosing a peer to
