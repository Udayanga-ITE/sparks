# User-space, Statically Defined Tracing (USDT) for Sparks Core

Sparks Core includes statically defined tracepoints to allow for more
...
**Note**: Sparks Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Sparks Core
...
Multiple tools can list the available tracepoints in a `sparksd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Sparks Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/sparksd
stap net        outbound_message 0x0000000000107c05 /src/sparksd
stap validation block_connected  0x00000000002fb10c /src/sparksd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Sparks Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/sparksd | grep NT_STAPSDT -A 4 -B 2
