...
Tracing (USDT) support for Sparks Core on Linux using. For more information on
USDT support in Sparks Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `sparksd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `sparksd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Sparks Core USDT documentation] on how to list available tracepoints in your
`sparksd` binary.

[Sparks Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/sparksd
```
...
Sparks P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/sparksd
```
...
In a different terminal, starting Sparks Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/sparksd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/sparksd
```
