...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if sparksd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("sparksd has not been built with USDT tracepoints enabled.")
...
class SparksTestFramework(BitcoinTestFramework):
    ...
    def set_sparks_test_params(self, num_nodes, masterodes_count, extra_args=None, evo_count=0):
