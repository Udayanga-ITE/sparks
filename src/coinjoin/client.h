...
public:
    CoinJoinWalletManager(ChainstateManager& chainman, CDeterministicMNManager& dmnman, CMasternodeMetaMan& mn_metaman,
                          const CTxMemPool& mempool, CSporkManager& spork_manager, const CMasternodeSync& mn_sync, const llmq::CInstantSendManager& isman,
...
public:
    ...
    bool DoAutomaticDenominating(ChainstateManager& chainman, CConnman& connman, const CTxMemPool& mempool,
                                 CSporkManager& spork_manager, bool fDryRun = false) EXCLUSIVE_LOCKS_REQUIRED(!cs_coinjoin);
...
public:
    ...
    explicit CCoinJoinClientManager(const std::shared_ptr<CWallet>& wallet, CoinJoinWalletManager& walletman,
                                    ...
                                    const CMasternodeSync& mn_sync, CSporkManager& spork_manager, const llmq::CInstantSendManager& isman,
                                    const std::unique_ptr<CCoinJoinClientQueueManager>& queueman, bool is_masternode) :
        m_wallet(wallet),
        m_walletman(walletman),
        m_dmnman(dmnman),
    ...
    /// Passively run mixing in the background according to the configuration in settings
    bool DoAutomaticDenominating(ChainstateManager& chainman, CConnman& connman, const CTxMemPool& mempool,
                                 CSporkManager& spork_manager, bool fDryRun = false) EXCLUSIVE_LOCKS_REQUIRED(!cs_deqsessions);
