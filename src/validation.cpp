...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!tx.IsSpecialTxVersion() || tx.nType != TRANSACTION_MNHF_SIGNAL) {
        if (!bypass_limits && !CheckFeeRate(ws.m_vsize, ws.m_modified_fees, state, tx.nType)) return false;
    }
...
MempoolAcceptResult AcceptToMemoryPool(CChainState& active_chainstate, const CTransactionRef& tx,
                                       int64_t accept_time, const CSporkManager& spork_manager, bool bypass_limits, bool test_accept)
                                             ...
                                             const Package& package, CSporkManager& spork_manager, bool test_accept)
{
    ...
    const auto result = [&]() EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
        ...
        if (test_accept) {
            ...
            return MemPoolAccept(pool, active_chainstate, spork_manager).AcceptMultipleTransactions(package, args);
        } else {
            auto args = MemPoolAccept::ATMPArgs::PackageChildWithParents(chainparams, GetTime(), coins_to_uncache);
            return MemPoolAccept(pool, active_chainstate, spork_manager).AcceptPackage(package, args);
        }
    }();
...
bool ChainstateManager::ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock>& block, CSporkManager& spork_manager, bool force_processing, bool* new_block)
{
    AssertLockNotHeld(cs_main);
    ...
    if (!ActiveChainstate().ActivateBestChain(state, spork_manager, block))
    ...
    try {
        ...
        while (num) {
            ...
            if (nTime > nNow - nExpiryTimeout) {
                LOCK(cs_main);
                const auto& accepted = AcceptToMemoryPool(active_chainstate, tx, nTime, spork_manager, /*bypass_limits=*/false, /*test_accept=*/false);
